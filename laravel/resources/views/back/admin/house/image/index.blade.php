                    ...
                    <table class="table table-striped table-hover table-sm">
                        ...
                        <tbody id="tbody-images">
                            ...
                            <tr id="{{ $imagen->id }}">
                                ...
                                <td>
                                    <a class="btn-show-image" data-imagen-id="{{ $imagen->id }}" href=""><img src="{{ asset('/public/img/houses/'.$imagen->nombre) }}" style="max-height: 100px;"></a>
                                </td>
                                <td>
        ...
        <div class="modal-content border-info animated fadeInLeftBig">
            ...
            <form id="form-create-image">
                <div class="modal-body">
                    ...
                    {{ csrf_field() }}
                    <input type="hidden" name="imagen_casa_id" value="{{ $casa->id }}">
                    <div class="form-group">
...
function changeStatus(checkbox)
{
    ...
    form.one('submit', function (e)
    {
        ...
        hideAlerts();
        var request = $.ajax({
            url: '<?= route('back.admin.house.image.status.save')?>',
            type: "POST",
            data: $(this).serialize()
        });
...
$('#modal-create-image').on('submit', '#form-create-image', function (e)
{
    ...
    var request = $.ajax({
        contentType: false,
        processData: false,
        url: "<?= route('back.admin.house.image.store')?>",
        type: "POST",
        data: formData
    });
    handleRequestDone(request);
...
$(document).on('click', ".btn-show-image", function (e)
{
    ...
    {
        var request = $.ajax({
            url: "{{ route('back.admin.house.image.show') }}",
            type: "GET",
            data: {
                id: id,
            }
...
$(document).on('click', ".btn-image-edit", function (e)
{
    ...
    {
        var request = $.ajax({
            url: "{{ route('back.admin.house.image.edit') }}",
            type: "GET",
            data: {
                id: id,
            }
...
$('#modal-image-edit').on('submit', '#form-image-edit', function (e)
{
    e.preventDefault();
    var request = $.ajax({
        url: "<?= route('back.admin.house.image.update')?>",
        type: "POST",
        data: $(this).serialize()
    });
    handleRequestDone(request);
...
$('.form-delete').on('submit', function (e)
{
    ...
    e.preventDefault();
    var request = $.ajax({
        url: "<?= route('back.admin.house.image.delete')?>",
        type: "POST",
        data: $(this).serialize()
    });
    handleRequestDone(request);
    ...
    handleRequestFailAndAlways(request, null);
});
</script>
@endsection